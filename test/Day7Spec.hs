...
spec = do
  describe "Day7" $ do
    context "parseAddres" $ do
      ...
      it "should return correct address" $ do
        parseAddress "asdf[qwer]xcvb[qer]asdf" `shouldBe` Address { seqs=["asdf", "xcvb", "asdf"], hnets=["qwer", "qer"] }

    context "supportsTLS" $ do
      it "should return correct results for test input" $ do
        map (supportsTLS) (map (parseAddress) day7TestInput) `shouldBe` [True, False, False, True]
