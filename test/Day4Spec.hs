  ...
  describe "Day4" $ do
    ...
    context "genCksum" $ do
      it "should return correct cksum" $ do
        genCksum (filter (/='-') "aaaaa-bbb-z-y-x-") `shouldBe` "abxyz"

      it "should return correct cksum" $ do
        genCksum (filter (/='-') "a-b-c-d-e-f-g-h-") `shouldBe` "abcde"

      it "should return correct cksum" $ do
        genCksum (filter (/='-') "not-a-real-room-") `shouldBe` "oarel"

      it "should return correct cksum" $ do
        genCksum (filter (/='-') "totally-real-room-") `shouldNotBe` "decoy"

    context "getValidRooms" $ do
      it "should return correct rooms" $ do
        let rooms = map parseRoom day4TestInput
            validrooms = map parseRoom $ take 3 day4TestInput
         in getValidRooms (rooms) `shouldBe` validrooms

    context "getSectorSum" $ do
      it "should generate correct sum for day4TestInput" $ do
        getSectorSum (getValidRooms (map parseRoom day4TestInput)) `shouldBe` 1514
