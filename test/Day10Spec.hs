...
spec = do
  describe "Day10" $ do
    context "parseInstr" $ do
      it "should parse value instr" $ do
        parseInstr "value 23 goes to bot 138" `shouldBe` Value { bot=138, val=23 }

      it "should parse bot instr" $ do
        let str = "bot 127 gives low to output 1 and high to bot 180"
            exp = Cmd { bot=127, lo=(Output 1), hi=(Bot 180) }
        parseInstr str `shouldBe` exp

      it "should parse bot instr" $ do
        let str = "bot 127 gives low to bot 1 and high to output 180"
            exp = Cmd { bot=127, lo=(Bot 1), hi=(Output 180) }
        parseInstr str `shouldBe` exp

    context "initState" $ do
      it "should return correct init state" $ do
        let instrs = map parseInstr day10TestInput
            bs = Map.fromList [(2, [5,2]), (1, [3])]
            os = Map.empty
            imap = Map.fromList [
              (2,Cmd{bot=2,lo=Bot 1,hi=Bot 0}),
              (1,Cmd{bot=1,lo=Output 1,hi=Bot 0}),
              (0,Cmd{bot=0,lo=Output 2,hi=Output 0})]
